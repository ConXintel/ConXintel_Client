(function () {
  ...
  function startOsintBg() {
    ...
    if (!host || !window.ConXHeroOsint || typeof window.ConXHeroOsint.initHost !== "function") return;
    osintDispose = window.ConXHeroOsint.initHost(host);
  }
  ...
  function normalizeProgress(progress) {
    ...
    var step = Number(progress.step) || 0;
    var total = Number(progress.total_steps) || 0;
    var pct = typeof progress.percent === "number" ? progress.percent : null;
    if ((pct === null || pct === 0) && step > 0 && total > 0) {
      pct = Math.min(99, (step / total) * 100);
    }
    if (pct === null) {
      return null;
    }
    return {
      percent: pct,
      step: step,
      total_steps: total || Math.max(step + 10, 1),
      label: progress.label || "",
    };
  }

  function progressIndeterminate(status, upstreamStatus, progress) {
    var normalized = normalizeProgress(progress);
    if (normalized && (normalized.percent > 0 || normalized.step > 0)) {
      return false;
    }
    return status === "queued" && upstreamStatus !== "running";
  }

  function forEachProgressTrack(fn) {
    var tracks = document.querySelectorAll(".search-job-progress-track");
  ...
  function setProgress(phase) {
    forEachProgressTrack(function (track) {
      if (phase === "queued") {
        track.setAttribute("data-progress-mode", "indeterminate");
        track.style.removeProperty("--progress");
      } else if (phase === "scanning") {
        track.setAttribute("data-progress-mode", "determinate");
        track.style.setProperty("--progress", "66");
  ...
  function setProgressPercent(percent, indeterminate) {
    forEachProgressTrack(function (track) {
      if (indeterminate) {
        track.setAttribute("data-progress-mode", "indeterminate");
        track.style.removeProperty("--progress");
      ...
      track.setAttribute("data-progress-mode", "determinate");
      track.style.setProperty("--progress", String(p));
    });
  }
  ...
  function setJobStatus(status, upstreamStatus, progress) {
    ...
    var normalized = normalizeProgress(progress);
    var pct = normalized ? normalized.percent : null;
    ...
    if (status === "done") {
      ...
      pct = 0;
    } else if (status === "running" && upstreamStatus === "queued" && !(normalized && normalized.step > 0)) {
      phase = "queued";
      ...
      if (normalized && normalized.label) {
        subtitle =
          ...
          " — " +
          normalized.label;
      } else if (normalized && normalized.step > 0) {
        subtitle = "Step " + normalized.step + " of " + normalized.total_steps;
      }
      if (pct !== null && pct > 0) {
        label = "Scanning — " + Math.round(pct) + "%";
      }
    ...
    setSteps(phase);
    if (pct !== null && status !== "failed") {
      setProgressPercent(pct, progressIndeterminate(status, upstreamStatus, progress));
    } else {
      setProgress(phase);
    }
    ...
    function tick() {
      ...
      fetch(pollJobUrl(jobId), {
        ...
        .then(function (result) {
          ...
          if (!progress && typeof data.progress_percent === "number") {
            progress = { percent: data.progress_percent };
          }
          setJobStatus(data.status, data.upstream_status, progress);
      ...
      .then(function (captchaOk) {
        ...
        fetch(jobsUrl, {
          ...
          .then(function (result) {
            ...
            setJobStatus(result.body.status || "queued", null, result.body.progress);
