"""ConX Client — team workspace that searches via the main ConX API (shared credits)."""
...
from conx_api import ConXApiError
from store import (
    DATA_DIR,
    REPORT_CONTEXT_MAX,
    append_audit,
...
app = Flask(__name__)
app.secret_key = os.environ.get("CONX_CLIENT_SECRET_KEY", "change-conx-client-secret-in-production")
...
SECURITY_PEPPER = (
    os.environ.get("CONX_CLIENT_SECURITY_PEPPER")
    or os.environ.get("CONX_CLIENT_SECRET_KEY")
    or "change-conx-client-secret-in-production"
)
...
CLIENT_SEARCH_JOB_TTL_SEC = max(60, int(os.environ.get("CONX_CLIENT_SEARCH_JOB_TTL", "3600")))
CLIENT_SEARCH_JOB_MAX = max(10, int(os.environ.get("CONX_CLIENT_SEARCH_JOB_MAX", "200")))
...
def _run_client_search_job(job_id: str) -> None:
    with app.app_context():
        ...
        if account and account.get("expired"):
            err = "ConX account expired. Renew on ConX Billing."
            with _CLIENT_SEARCH_JOBS_LOCK:
...
def _setup_complete(store: Dict[str, Any]) -> bool:
    ...
    return bool(
        ...
        and (cfg.get("conx_url") or "").strip()
        and (cfg.get("api_token") or "").strip()
        and (cfg.get("owner_password_hash") or "").strip()
    )


def _conx_credentials(store: Dict[str, Any]) -> tuple[str, str]:
    cfg = _config(store)
    return (cfg.get("conx_url") or "").strip().rstrip("/"), (cfg.get("api_token") or "").strip()


def _apply_conx_api_config(
    store: Dict[str, Any],
    *,
    conx_url: str,
    api_token: str,
) -> tuple[Optional[str], Optional[Dict[str, Any]]]:
    """Verify ConX API credentials and update store config. Returns (error, account)."""
    conx_url = (conx_url or "").strip().rstrip("/")
    api_token = (api_token or "").strip()
    if not conx_url or not api_token:
        return "ConX server URL and API token are required.", None
    try:
        account = conx_api.verify_api_access(conx_url, api_token)
    except ConXApiError as exc:
        hint = ""
        if exc.code == "invalid_token":
            hint = " Generate a new token in ConX → API Access and paste it here."
        return f"Could not verify ConX API: {exc}{hint}", None
    if account.get("expired"):
        return "The ConX account for this API key is expired. Renew on ConX Billing first.", None
    if not account.get("api_access_enabled"):
        return (
            "API access is not enabled for this ConX account. Enable API on an Agency plan "
            "or turn on standalone API access in ConX admin.",
            None,
        )
    cfg = _config(store)
    cfg.update(
        {
            "conx_url": conx_url,
            "api_token": api_token,
            "conx_owner_email": account.get("email") or cfg.get("conx_owner_email") or "",
            "conx_package": account.get("package") or cfg.get("conx_package") or "",
        }
    )
    store["config"] = cfg
    return None, account
...
def _fetch_account_with_error(store: Dict[str, Any]) -> tuple[Optional[Dict[str, Any]], str]:
    ...
    if not base or not token:
        return None, "ConX URL or API token is not configured."
    try:
        account = conx_api.get_account_or_probe(base, token)
...
def _intel_image_origin() -> str:
    return (os.environ.get("CONX_CLIENT_INTEL_IMAGE_ORIGIN") or "").strip().rstrip("/")
...
def setup():
    ...
    if request.method == "POST":
        conx_url = (request.form.get("conx_url") or "").strip().rstrip("/")
        ...
        if not conx_url or not api_token or not owner_username or not owner_password:
            flash("All setup fields are required.", "error")
            return render_template("setup.html", config=_config(store))
        err, account = _apply_conx_api_config(store, conx_url=conx_url, api_token=api_token)
        if err:
            flash(err, "error")
            return render_template("setup.html", config=_config(store))
        ...
        if account.get("account_endpoint_missing"):
            flash(
                "API token verified. Restart the main ConX app (python app.py) for full "
...
def settings():
    ...
    if request.method == "POST":
        ...
        conx_url = (request.form.get("conx_url") or cfg.get("conx_url") or "").strip().rstrip("/")
        ...
        if uploaded_logo:
            ...
            audit_detail = "Organization logo reset to ConX default"
        else:
            audit_detail = f"ConX API linked: {(account or {}).get('email', '')}"
...
def dashboard_api_live():
    ...
    base, _token = _conx_credentials(store)
    health = conx_api.fetch_search_health(base) if base else {"reachable": False, "degraded": True}
    return jsonify(
        {
            "ok": True,
...
def dashboard():
    ...
    base, _token = _conx_credentials(store)
    search_health = conx_api.fetch_search_health(base) if base else None
...
def team_dashboard():
    ...
    if account:
        ...
        if monthly_limit > 0 and monthly_used >= max(1, int(monthly_limit * 0.9)):
            flags.append({"level": "warn", "title": "Shared search cap almost reached", "detail": f"{monthly_used}/{monthly_limit} used on the ConX account."})
...
def search_jobs_create():
    ...
    if account and account.get("expired"):
        return jsonify({"ok": False, "error": "ConX account expired. Renew on ConX Billing."}), 400
...
def search_jobs_poll(job_id: str):
    """Poll client-side search job; applies session when ConX API search completes."""
    jid = (job_id or "").strip()
    actor = _actor_label()
    apply_now = False
...
def api_account_snapshot():
    ...
    account = _fetch_account(store)
    if not account:
        return jsonify({"ok": False, "error": "Could not reach ConX API."}), 502
    return jsonify({"ok": True, "account": account})


if __name__ == "__main__":
    port = int(os.environ.get("CONX_CLIENT_PORT", "8990"))
    debug = os.environ.get("CONX_CLIENT_DEBUG", "").strip().lower() in ("1", "true", "yes")
